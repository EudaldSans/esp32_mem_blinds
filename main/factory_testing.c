...
void __URI_get_blinds(httpd_req_t * xReq);
void __URI_post_blinds(httpd_req_t * xReq);
...
bool _factorytest_webStart(SERVER_OBJ* xWebUi)
{
    ...
    if (HTTP_StartWebserver(xWebUi, FACTORY_TEST_HTTP_SERVER_ID, FACTORY_TEST_HTTP_PRIORITY, FACTORY_TEST_HTTP_PORT, 20) == true)
    {
        ...
        HTTP_RegisterUri(xWebUi, HTTP_GET, "/blinds", __URI_get_blinds);
        HTTP_RegisterUri(xWebUi, HTTP_POST, "/blinds", __URI_post_blinds);
...
// char key[10];
    ...
    payload_str = cJSON_Print(payload);
    ESP_LOGI(TAG_FACTORY_TESTING, "GET /blinds\n%s\n", payload_str);
    HTTP_ResponseSend(xReq, payload_str, HTTP_CONTENT_JSON);
    cJSON_Delete(payload);
}
...
int index;
    ...
    ESP_LOGI(TAG_FACTORY_TESTING, "POST /blinds\n%s", cData);
    if (bLenGetData) {
        reqData= cJSON_Parse(cData);
        cJSON_ArrayForEach(blind, reqData) {
            if(cJSON_IsObject(blind)) {
                index = atoi(blind->string);
                if (index != 0) continue;

                if(cJSON_HasObjectItem(blind, "status")) {
...
cJSON * xPayload = cJSON_CreateObject();
    ...
    cJSON_AddNumberToObject(xPayload, "Ton", SyncTest_GetTon());
